...
int pt2(FILE* fp)
{
     ...
     */
    /* Now the buffer comes in handy! I also saved the \n chars, so i will count from
     * the end of the line, work my way backwards, and the top num is MSC, therefore
     i should be able to just construct the num straight into an int
     remember counting is 1 based, so w-1 for column num
     */
    ...
    for(int col=w-1; col>=0; --col) // This is every char (col), starting at the end
    {
        TRACE("col %i out of %i", col, w);
        ...
        if (dpos > 0) {
            ...
            DEBUG("col=%i -> number %llu", col, value);
        }        

        op = buffer[w*op_row+col];
        ...
        if ((op == '+' || op == '*') && num_count > 0) {
            ...
            INFO("Problem result (%c): %llu", op, acc);
            final_score += acc;
