...
int pt2(FILE* fp){
    ...
    uint16_t v = extract_value(find_wire("a"));

    /* They only place part 2 differs
     * Overriding instruction with that was found in part 1
     * Have to reset the cache and run it again
     * */
    wire *b = find_wire("b");
    sprintf(b->instructions[0], "%i", v);

    memset(c,0,CACHE_SIZE);
    cache_count=0;

    v = extract_value(find_wire("a"));
